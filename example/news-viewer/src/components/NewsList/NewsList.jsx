...
const NewsList = ({ category }) => {
  ...
  useEffect(() => {
    //useEffect 이용해서 컴포넌트가 처음 랜더링 되는 시점에 api 요청
    //async 사용하는 함수 따로 선언
    const fetchData = async () => {
      //useEffect에서 반환하는 값은 뒷정리 함수라서 등록하는 함수는 async안됨
      setLoading(true); //함수 호출되면 loading true
      try {
        const query = category === 'all' ? '' : `&category=${category}`;
        const response = await Axios.get(
          //get 요청
          `https://newsapi.org/v2/top-headlines?country=kr${query}&apiKey=a0dde6dd23de4de6a2204df46df33894`,
        );
  ...
  if (!articles) {
    //데이터가 없을 때, map 함수를 사용할수가 없기 때문에 랜더링 과정에서 오류가 발생함
    return null;
  }
  //article 값이 유효할 때
  return (
    <NewsListBlock>
      {articles.map((
        article, //map 함수를 이용해 각 article 그려줌
      ) => (
